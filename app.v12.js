...
function logDbg(...a){console.log(...a);if(!dbg)return;dbg.style.display='block';dbg.textContent=a.map(x=>typeof x==='string'?x:JSON.stringify(x)).join(' | ');}

if('serviceWorker' in navigator){ addEventListener('load', ()=>navigator.serviceWorker.register('sw.v12.js').catch(console.warn)); }
...
(async function init(){
  ...
  }catch(err){ logDbg('Init error', err.message); syncStatus.textContent='Init error. Try #reset.'; syncStatus.className='status status-bad'; }
})();

async function loadCodes(){
  const url = localStorage.getItem('pr.webAppUrl') || DEFAULT_WEB_APP_URL;
  try{
    let r = await fetch(url+'?action=codes', {cache:'no-store', mode:'cors'});
    if(!r.ok) throw new Error('codes '+r.status);
    let j = await r.json();
    if(!(j && j.ok && j.codes)) throw new Error('codes format');
    codes = { expense: j.codes.expense||[], income: j.codes.income||[] };
    localStorage.setItem('pr.codes', JSON.stringify(codes));
  }catch(e){
    try{
      await fetch(url+'?action=bootstrap', {cache:'no-store'});
      let r2 = await fetch(url+'?action=codes', {cache:'no-store'});
...
async function onAddCode(){
  ...
  try{
    const body = JSON.stringify({ action:'addCode', type:entryType, code:val });
...
async function onSubmit(ev){
  ...
  try{
    ...
    const url = localStorage.getItem('pr.webAppUrl') || DEFAULT_WEB_APP_URL;
    const body = JSON.stringify({ action:'record', data:payload });
    const res = await fetch(url, { method:'POST', headers:{'Content-Type':'application/json'}, body, cache:'no-store' });
  ...
  }catch(err){ saveMsg.textContent = err.message || 'Load failed'; saveMsg.style.color = '#b91c1c'; logDbg('Save error', err.message||err); }
