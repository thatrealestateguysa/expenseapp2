...
if('serviceWorker' in navigator) addEventListener('load', ()=>navigator.serviceWorker.register('sw.v13.js'));
...
async function loadCodes(){
  ...
  try{
    const r=await fetch(url+'?action=codes',{cache:'no-store'});
    const j=await r.json();
    if(j&&j.ok&&j.codes) codes=j.codes;
    localStorage.setItem('pr.codes', JSON.stringify(codes));
  }catch(e){
    const c=localStorage.getItem('pr.codes'); if(c) codes=JSON.parse(c);
  }
}
...
async function onAddCode(){
  ...
  try{
    let r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:bodyJson});
    const js=await r.json(); if(js&&js.ok&&js.codes){ codes=js.codes; localStorage.setItem('pr.codes',JSON.stringify(codes)); populateCodes(entryType); costCodeSelect.value=val; }
  }catch(_e){
    let r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body:'payload='+encodeURIComponent(bodyJson)});
    try{ const js=await r.json(); if(js&&js.ok&&js.codes){ codes=js.codes; localStorage.setItem('pr.codes',JSON.stringify(codes)); populateCodes(entryType); costCodeSelect.value=val; } }catch(_ ){}
  }
}
...
async function healthCheck(alsoReloadCodes){
  ...
  const url = localStorage.getItem('pr.webAppUrl') || DEFAULT_WEB_APP_URL;
  try{
    const r = await fetch(url+'?action=summary', {cache:'no-store'});
    if(r.ok){ const j = await r.json(); if(j && typeof j.income==='number'){ totalIncomeEl.textContent=money(j.income||0); totalExpensesEl.textContent=money(j.expenses||0); netBalanceEl.textContent=money(j.net||0); } }
...
async function onSubmit(ev){
  ...
  try{
    ...
    const url = localStorage.getItem('pr.webAppUrl') || DEFAULT_WEB_APP_URL;
    try{
      const r = await fetch(url, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action:'record', data:payload }) });
      if(!r.ok) throw new Error('HTTP '+r.status);
      const js = await r.json(); if(!js.ok) throw new Error(js.error||'Server error');
      onSaved(js, payload);
    }catch(e1){
      const body = 'payload='+encodeURIComponent(JSON.stringify({action:'record',data:payload}));
      const r2 = await fetch(url, { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'}, body });
      if(!r2.ok) throw new Error('HTTP '+r2.status);
      const js2 = await r2.json(); if(!js2.ok) throw new Error(js2.error||'Server error');
      onSaved(js2, payload);
    }
  }catch(err){
    saveMsg.textContent = err.message || 'Failed to save'; saveMsg.style.color = '#b91c1c';
  }finally{ saveBtn.disabled=false; }
}
function onSaved(server, payload){
  const entry = { ...payload, receiptUrl: server.receiptUrl, id: server.rowNumber || Date.now() };
...
async function buildPayload(){
  ...
  let receipt=null;
  if(chosenFile){
    const dataUrl=await downscaleToJpegDataUrl(chosenFile,1400,0.8);
...
function clearForm(){
  descriptionEl.value=''; amountEl.value=''; costCodeSelect.value=''; chosenFile=null; preview.innerHTML='';
}
function money(n){
  try{ return new Intl.NumberFormat('en-ZA',{style:'currency',currency:'ZAR'}).format(n); }catch(_ ){ return 'R'+Number(n).toFixed(2); }
}
