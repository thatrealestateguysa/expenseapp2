...
console.log('PR Expenses v6 boot');
...
(async function init(){
  try {
    ...
    syncStatus.textContent = 'App failed to initialize. Open with #reset once.';
...
async function healthCheck(){
  ...
  try{
    ...
    if(js && typeof js.income==='number'){ totalIncomeEl.textContent=formatMoney(js.income||0); totalExpensesEl.textContent=formatMoney(js.expenses||0); netBalanceEl.textContent=formatMoney(js.net||0); }
  }catch(err){
...
document.getElementById("entryForm").addEventListener("submit", async (ev)=>{
  ...
  try{
    ...
    saveMsg.textContent=(err && err.message) ? err.message : "Load failed"; saveMsg.style.color="#b91c1c";
